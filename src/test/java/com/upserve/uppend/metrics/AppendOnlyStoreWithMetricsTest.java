...
public class AppendOnlyStoreWithMetricsTest {
    ...
    public void testReadFlushed() throws Exception {
        ...
        assertArrayEquals(
                Arrays.asList("first", "second").toArray(),
                instance.readFlushed("partition", "key1").map(String::new).toArray()
        );
        assertNotEquals(0, metrics.timer(AppendOnlyStoreWithMetrics.READ_TIMER_METRIC_NAME).getCount());
        assertEquals(11, metrics.meter(AppendOnlyStoreWithMetrics.READ_BYTES_METER_METRIC_NAME).getCount());
    }
    ...
    public void testReadSequentialFlushed() throws Exception {
        ...
        assertArrayEquals(
                Arrays.asList("first", "second").toArray(),
                instance.readSequentialFlushed("partition", "key1").map(String::new).toArray()
        );
        assertNotEquals(0, metrics.timer(AppendOnlyStoreWithMetrics.READ_TIMER_METRIC_NAME).getCount());
        assertEquals(11, metrics.meter(AppendOnlyStoreWithMetrics.READ_BYTES_METER_METRIC_NAME).getCount());
    }
    ...
    public void testReadLastFlushed() throws Exception {
        ...
        assertEquals(
                "last",
                new String(instance.readLastFlushed("partition", "key1"))
        );
        assertNotEquals(0, metrics.timer(AppendOnlyStoreWithMetrics.READ_TIMER_METRIC_NAME).getCount());
        assertEquals(4, metrics.meter(AppendOnlyStoreWithMetrics.READ_BYTES_METER_METRIC_NAME).getCount());
    }
